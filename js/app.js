...
// HEADER
// ============================================
function renderHeader(title, showBack = false, backPage = null) {
  const state = getState();
  const isLoggedIn = state.currentRole !== null;

  return `
    <header class="app-header">
      ${showBack ? `
        <button class="app-header__back" id="header-back" data-page="${backPage}">
      ...
      <div class="app-header__actions">
        ...
        ${isLoggedIn ? `<button class="app-header__action-btn app-header__action-btn--danger" id="btn-logout" aria-label="Log out" title="Log out">⏻</button>` : ''}
      </div>
    </header>
  `;
}
    ...
    <nav class="bottom-nav">
      ${items.map(item => `
        <button class="bottom-nav__item ${activePage === item.page ? 'bottom-nav__item--active' : ''}"
                id="nav-${item.id}" data-page="${item.page}">
  ...
  return `
    <div class="role-select">
      <div class="role-select__hero">
        ...
        <h1 class="role-select__title">Medical Care<br>Connect</h1>
      ...
      <div class="section-title" style="margin-top:0; text-align:center;">Continue as</div>
    ...
    <main class="page">
      ...
      ${state.ambulanceRequested ? `
        ...
        <div class="tracking-card" id="active-tracking-card" style="cursor:pointer;">
          <div class="badge badge--info mb-2"><span class="badge__dot"></span> Live</div>
          <div class="tracking-card__eta">${state.ambulanceEta || 7} min</div>
          <div class="tracking-card__eta-label">Ambulance arriving</div>
          <p class="text-xs text-muted mt-2">Tap to view full tracking →</p>
        </div>
      ` : ''}
    </main>
...
registerEvents('patient-hospitals', () => {
  ...
  document.querySelectorAll('.hospital-card').forEach(card => {
    card.addEventListener('click', () => {
      setState({ selectedHospital: card.dataset.hospitalId });
  ...
  return `
    <div class="hospital-card hospital-card--${h.status} card--interactive" data-hospital-id="${h.id}" role="button" tabindex="0">
      ...
      <div class="hospital-card__stats">
        <div class="hospital-card__stat"><span class="hospital-card__stat-icon">📍</span> ${h.distance} km</div>
        <div class="hospital-card__stat"><span class="hospital-card__stat-icon">⏱️</span> ~${h.waitTime} min</div>
        <div class="hospital-card__stat"><span class="hospital-card__stat-icon">🛏️</span> ${h.erCapacity.available}/${h.erCapacity.total} ER</div>
      </div>
    </div>
  `;
}
...
registerRoute('patient-hospital-detail', () => {
  ...
  const capacityColorVar = h.status === 'full' ? 'full' : h.status === 'limited' ? 'limited' : 'available';
  ...
  return `
    ...
    <main class="page">
      ...
      <div class="stat-row mb-4">
        <div class="stat">
          <div class="stat__value" style="color: var(--color-capacity-${capacityColorVar})">${h.erCapacity.available}</div>
          <div class="stat__label">ER Beds</div>
        </div>
        <div class="stat">
          <div class="stat__value" style="color: var(--color-capacity-${h.icuCapacity.available <= 2 ? (h.icuCapacity.available === 0 ? 'full' : 'limited') : 'available'})">${h.icuCapacity.available}</div>
      ...
      <div class="card">
        <div class="hospital-detail__info">
          <div class="info-row">
            ...
            <div class="info-row__content"><div class="info-row__label">Phone</div><div class="info-row__value">${h.phone}</div></div>
          </div>
          <div class="info-row">
            <div class="info-row__icon">⏱️</div>
            <div class="info-row__content"><div class="info-row__label">Estimated Wait</div><div class="info-row__value">~${h.waitTime} minutes</div></div>
          </div>
          <div class="info-row">
            <div class="info-row__icon">⭐</div>
            <div class="info-row__content"><div class="info-row__label">Rating</div><div class="info-row__value">${h.rating} / 5.0</div></div>
          </div>
          <div class="info-row">
            <div class="info-row__icon">🏷️</div>
            <div class="info-row__content"><div class="info-row__label">Departments</div><div class="info-row__value">${h.departments.join(', ')}</div></div>
          </div>
        </div>
      </div>
      ...
      <button class="btn btn--primary btn--block btn--lg mt-6" id="btn-get-directions">🗺️ Get Directions</button>
      <button class="btn btn--outline btn--block mt-2" id="btn-request-ambulance-detail">🚑 Request Ambulance Here</button>
    </main>
    ${renderBottomNav('patient', 'patient-hospitals')}
  `;
});
...
registerEvents('patient-hospital-detail', () => {
  ...
  document.getElementById('btn-get-directions')?.addEventListener('click', () => {
    if (h) window.open(`https://www.google.com/maps/dir/?api=1&destination=${h.lat},${h.lng}`, '_blank');
  });
  document.getElementById('btn-request-ambulance-detail')?.addEventListener('click', () => navigate('patient-ambulance-request'));
  attachNavEvents();
  attachGlobalEvents();
});
...
function renderCapacitySection(name, cap) {
  ...
  return `
    <div class="card mb-2" style="padding: var(--space-3) var(--space-4);">
...
registerRoute('patient-ambulance-request', () => {
  ...
  if (state.ambulanceRequested) {
    return `
      ...
      <main class="page">${renderTrackingView()}</main>
  ...
  return `
    ...
    <main class="page">
      <div class="text-center" style="padding: var(--space-6) 0;">
        ...
        <h1 class="page__title">Request Ambulance</h1>
        <p class="page__subtitle" style="max-width:280px; margin: var(--space-2) auto var(--space-5);">
        ...
        <div class="card mb-4" style="text-align:left;">
          <div class="card__header">
            <div>
              <div class="card__title">Nearest Ambulance</div>
              <div class="card__description">${nearestAmb.vehicleNumber}</div>
            </div>
            <div class="badge badge--available"><span class="badge__dot"></span> Ready</div>
          </div>
          <div class="flex gap-4 mt-2">
            <span class="text-xs text-secondary">👤 ${nearestAmb.driver}</span>
            <span class="text-xs text-secondary">📍 ~2.1 km away</span>
          </div>
        </div>
        ` : ''}

        <button class="btn btn--danger btn--block btn--lg" id="btn-confirm-ambulance">🚨 Confirm Request</button>
...
registerRoute('patient-tracking', () => {
  ...
  return `
    ...
    <main class="page">
      ${renderTrackingView()}
      <button class="btn btn--apple btn--block mt-4" id="btn-share-status">📤 Share Status with Family</button>
    </main>
    ${renderBottomNav('patient', 'patient-tracking')}
  `;
});
...
registerEvents('patient-tracking', () => {
  ...
  const amb = ambulances.find(a => a.id === tracking?.ambulanceId);

  if (h && amb) {
    initMap('map-tracking', {
      center: { lat: (h.lat + amb.lat) / 2, lng: (h.lng + amb.lng) / 2 },
      zoom: 13,
      markers: [
        { lat: h.lat, lng: h.lng, title: h.shortName, label: 'Destination' },
        { lat: amb.lat, lng: amb.lng, title: '🚑 Ambulance', label: `ETA: ${tracking.eta} min` },
      ],
    });
  }
...
function renderTrackingView() {
  ...
  return `
    ...
    <div class="card mt-4" style="padding:var(--space-3) var(--space-4);">
      <div class="text-xs text-muted mb-1">TRACKING CODE</div>
      <div style="font-size:var(--text-lg);font-weight:700;letter-spacing:0.1em;color:var(--color-brand);">${code}</div>
    </div>
    ...
    <div class="tracking-steps">
      ${tracking?.steps.map(step => `
        <div class="tracking-step ${step.completed ? 'tracking-step--completed' : ''} ${step.active ? 'tracking-step--active' : ''}">
          <div class="tracking-step__indicator">${step.completed ? '✓' : step.active ? '●' : '○'}</div>
    ...
    <div class="card mt-4" style="padding:var(--space-3) var(--space-4);">
      <div class="text-xs text-muted mb-1">DESTINATION</div>
  ...
  return `
    ...
    <main class="page">
      <div class="patient-hero">
        ...
        <p class="text-sm text-secondary mb-4">Enter the tracking code shared by the patient.</p>
      </div>
      ...
      <div class="privacy-banner">
        ...
        <span class="privacy-banner__text">The patient shares a tracking code after requesting an ambulance.</span>
      </div>

      <div class="section-title">Demo Codes</div>
      <div class="card card--interactive mb-2" id="demo-code-1" style="padding:var(--space-3) var(--space-4);">
        <div class="flex justify-between items-center">
          <div>
            <div style="font-weight:600;font-size:var(--text-sm);">TRACK-001</div>
            <div class="text-xs text-secondary">Arjun Mehta — En route to GMCH-32</div>
          </div>
          <div class="badge badge--info"><span class="badge__dot"></span> Live</div>
        </div>
      </div>
      <div class="card card--interactive" id="demo-code-2" style="padding:var(--space-3) var(--space-4);">
        <div class="flex justify-between items-center">
          <div>
            <div style="font-weight:600;font-size:var(--text-sm);">TRACK-002</div>
            <div class="text-xs text-secondary">Priya Kaur — Pickup in progress</div>
          </div>
          <div class="badge badge--limited"><span class="badge__dot"></span> Pending</div>
        </div>
      </div>
    </main>
...
registerRoute('family-tracking', () => {
  const state = getState();
  const tracking = trackingCodes[state.trackingCode];
  if (!tracking) {
    return `
      ...
      <main class="page text-center" style="padding-top:var(--space-16);">
        ...
        <p class="page__subtitle">Enter a valid tracking code.</p>
  ...
  return `
    ${renderHeader('Tracking: ' + state.trackingCode, true, 'family-home')}
    <main class="page">
      ...
      <div class="card mt-4" style="padding:var(--space-3) var(--space-4);">
        <div class="text-xs text-muted mb-1">DESTINATION</div>
        <div style="font-weight:600;">${h?.name || 'Hospital'}</div>
        <div class="text-xs text-secondary mt-1">${h?.address || ''} · 📞 ${h?.phone || ''}</div>
      </div>
    </main>
    ${renderBottomNav('family', 'family-tracking')}
  `;
...
registerRoute('ambulance-home', () => {
  ...
  const assignedHospital = hospitals[2]; // Fortis Mohali

  return `
    ${renderHeader('MedConnect Driver')}
    <main class="page">
      <h1 class="page__title">Welcome, Driver 🚑</h1>
      <p class="page__subtitle">Manage your status and view assignments.</p>

      <div class="driver-status">
        <div class="driver-status__info">
          <span style="font-size:1.3rem;">${statusEmojis[state.driverStatus]}</span>
          <div>
            <div class="driver-status__label">Your Status</div>
            <div class="driver-status__value">${statusLabels[state.driverStatus]}</div>
          </div>
        </div>
        <select id="driver-status-select" style="width:auto; padding:var(--space-2) var(--space-3); font-size:var(--text-sm); border-radius:var(--radius-md);">
          ${['available', 'en_route', 'offline'].map(s => `<option value="${s}" ${state.driverStatus === s ? 'selected' : ''}>${statusLabels[s]}</option>`).join('')}
        </select>
      </div>
      ...
      ${state.driverStatus !== 'offline' ? `
        ...
        <div class="assignment-card">
          ...
          <div class="assignment-card__meta">
            ...
            <div class="assignment-card__meta-item">⏱️ ~${Math.round(assignedHospital.distance * 3)} min</div>
          </div>
        </div>
      ...
      ` : `
        <div class="text-center" style="padding:var(--space-12) 0;">
          ...
          <p class="text-sm text-secondary mt-2">Change status to "Available" for assignments.</p>
        </div>
      `}

      <div class="section-title">Fleet Overview</div>
      <div class="stat-row">
        <div class="stat"><div class="stat__value text-success">${ambulances.filter(a => a.status === 'available').length}</div><div class="stat__label">Available</div></div>
        <div class="stat"><div class="stat__value text-warning">${ambulances.filter(a => a.status === 'en_route').length}</div><div class="stat__label">En Route</div></div>
        <div class="stat"><div class="stat__value text-danger">${ambulances.filter(a => a.status === 'offline').length}</div><div class="stat__label">Offline</div></div>
      </div>
    </main>
...
registerEvents('ambulance-home', () => {
  const assignedHospital = hospitals[2];
  initMap('map-driver', {
    center: { lat: assignedHospital.lat, lng: assignedHospital.lng },
    zoom: 14,
    markers: [{ lat: assignedHospital.lat, lng: assignedHospital.lng, title: assignedHospital.shortName, label: assignedHospital.address }],
  });
...
registerRoute('ambulance-assignment', () => {
  const h = hospitals[2];
  return `
    ${renderHeader('Assignment', true, 'ambulance-home')}
    <main class="page">
      <div class="assignment-card mb-4">
        <div class="assignment-card__label">Current Assignment</div>
        <div class="assignment-card__hospital">${h.name}</div>
        <div class="assignment-card__address">${h.address}</div>
        <div class="assignment-card__meta mt-4">
          <div class="assignment-card__meta-item">📍 ${h.distance} km</div>
          <div class="assignment-card__meta-item">🛏️ ${h.erCapacity.available} ER beds</div>
          <div class="assignment-card__meta-item">📞 ${h.phone}</div>
        </div>
      </div>
      ...
      <button class="btn btn--primary btn--block btn--lg mt-6" id="btn-navigate">🗺️ Open Navigation</button>
    </main>
...
registerRoute('admin-dashboard', () => {
  const totalER = hospitals.reduce((s, h) => s + h.erCapacity.available, 0);
  const totalICU = hospitals.reduce((s, h) => s + h.icuCapacity.available, 0);
  const active = ambulances.filter(a => a.status !== 'offline').length;
  ...
  return `
    ...
    <main class="page">
      ...
      <div class="admin-grid">
        <div class="admin-stat-card"><div class="admin-stat-card__value text-success">${totalER}</div><div class="admin-stat-card__label">ER Beds Free</div></div>
        <div class="admin-stat-card"><div class="admin-stat-card__value text-warning">${totalICU}</div><div class="admin-stat-card__label">ICU Beds Free</div></div>
        <div class="admin-stat-card"><div class="admin-stat-card__value text-brand">${active}</div><div class="admin-stat-card__label">Active Ambulances</div></div>
        <div class="admin-stat-card"><div class="admin-stat-card__value">${hospitals.length}</div><div class="admin-stat-card__label">Hospitals</div></div>
      </div>
      ...
      <div class="hospitals-list">
        ${hospitals.map(h => `
          <div class="hospital-card hospital-card--${h.status}" style="cursor:default;">
            ...
            <div class="hospital-card__stats">
              <div class="hospital-card__stat"><span class="hospital-card__stat-icon">🛏️</span> ER: ${h.erCapacity.available}/${h.erCapacity.total}</div>
              <div class="hospital-card__stat"><span class="hospital-card__stat-icon">🏥</span> ICU: ${h.icuCapacity.available}/${h.icuCapacity.total}</div>
              <div class="hospital-card__stat"><span class="hospital-card__stat-icon">🛌</span> Gen: ${h.generalBeds.available}/${h.generalBeds.total}</div>
            </div>
          </div>
        `).join('')}
      </div>
...
registerEvents('admin-dashboard', () => { attachNavEvents(); attachGlobalEvents(); });
...
registerRoute('admin-capacity', () => {
  return `
    ${renderHeader('Capacity', true, 'admin-dashboard')}
    <main class="page">
      ...
      ${hospitals.map(h => `
        <div class="capacity-control" data-hospital-id="${h.id}">
          ...
          <div class="mb-4">
            <div class="capacity-control__header"><span class="capacity-control__name">🚨 Emergency Room</span><span class="capacity-control__count">${h.erCapacity.available} / ${h.erCapacity.total}</span></div>
            <input type="range" class="range-slider" min="0" max="${h.erCapacity.total}" value="${h.erCapacity.available}" data-type="er" data-hospital="${h.id}" />
          </div>
          <div class="mb-4">
            <div class="capacity-control__header"><span class="capacity-control__name">🏥 ICU</span><span class="capacity-control__count">${h.icuCapacity.available} / ${h.icuCapacity.total}</span></div>
            <input type="range" class="range-slider" min="0" max="${h.icuCapacity.total}" value="${h.icuCapacity.available}" data-type="icu" data-hospital="${h.id}" />
          </div>
          <div>
            <div class="capacity-control__header"><span class="capacity-control__name">🛌 General Beds</span><span class="capacity-control__count">${h.generalBeds.available} / ${h.generalBeds.total}</span></div>
            <input type="range" class="range-slider" min="0" max="${h.generalBeds.total}" value="${h.generalBeds.available}" data-type="general" data-hospital="${h.id}" />
          </div>
        </div>
      `).join('')}
    </main>
...
registerEvents('admin-capacity', () => {
  document.querySelectorAll('.range-slider').forEach(slider => {
    slider.addEventListener('input', (e) => {
      const hospital = hospitals.find(h => h.id === e.target.dataset.hospital);
      if (!hospital) return;
      const val = parseInt(e.target.value);
      const type = e.target.dataset.type;
      if (type === 'er') hospital.erCapacity.available = val;
      if (type === 'icu') hospital.icuCapacity.available = val;
      if (type === 'general') hospital.generalBeds.available = val;
      const pct = (hospital.erCapacity.available + hospital.icuCapacity.available) / (hospital.erCapacity.total + hospital.icuCapacity.total);
      ...
      if (countEl) countEl.textContent = `${val} / ${e.target.max}`;
    });
  });
  attachNavEvents();
  attachGlobalEvents();
...
registerRoute('admin-ambulances', () => {
  const emojis = { available: '🟢', en_route: '🟡', offline: '🔴' };
  const labels = { available: 'Available', en_route: 'En Route', offline: 'Offline' };
  const badgeCls = { available: 'badge--available', en_route: 'badge--limited', offline: 'badge--full' };

  return `
    ${renderHeader('Fleet', true, 'admin-dashboard')}
    <main class="page">
      <h1 class="page__title">Ambulance Queue</h1>
      <p class="page__subtitle">Monitor all ambulances</p>
      ...
      <div class="ambulance-queue">
        ${ambulances.map(a => {
    const dest = a.assignedHospital ? hospitals.find(h => h.id === a.assignedHospital) : null;
    return `
            <div class="queue-item">
              <div class="queue-item__icon" style="background: ${a.status === 'available' ? 'var(--color-success-bg-strong)' : a.status === 'en_route' ? 'var(--color-warning-bg-strong)' : 'var(--color-danger-bg-strong)'}">
                ${emojis[a.status]}
              </div>
              <div class="queue-item__info">
                <div class="queue-item__name">${a.vehicleNumber}</div>
                <div class="queue-item__detail">👤 ${a.driver}${dest ? ` · → ${dest.shortName}` : ''}</div>
              </div>
              ${a.eta ? `<div class="queue-item__eta">${a.eta} min</div>` : `<div class="badge ${badgeCls[a.status]}">${labels[a.status]}</div>`}
            </div>
          `;
  }).join('')}
      </div>
...
registerEvents('admin-ambulances', () => { attachNavEvents(); attachGlobalEvents(); });
...
function attachNavEvents() {
  ...
  document.getElementById('header-back')?.addEventListener('click', () => {
    const page = document.getElementById('header-back').dataset.page;
...
function attachGlobalEvents() {
  ...
  document.getElementById('btn-logout')?.addEventListener('click', () => {
    // Restore hospital data
    hospitals.forEach(h => {
      const o = originalHospitals.find(x => x.id === h.id);
      if (o) { h.erCapacity = { ...o.erCapacity }; h.icuCapacity = { ...o.icuCapacity }; h.generalBeds = { ...o.generalBeds }; h.status = o.status; }
    });
    resetState();
    renderCurrentPage(app);
    showToast({ title: 'Logged Out', message: 'You have been logged out.', icon: '👋', type: 'info', duration: 2500 });
  });
  ...
  document.getElementById('demo-reset-btn')?.addEventListener('click', () => {
    if (confirm('Reset all demo data?')) {
      ...
      resetState();
      renderCurrentPage(app);
      showToast({ title: 'Demo Reset', message: 'All data restored.', icon: '🔄', type: 'info' });
    }
  });

  // Notifications
  document.getElementById('btn-notifications')?.addEventListener('click', () => {
    showToast({ title: 'Notifications', message: 'No new notifications.', icon: '🔔', type: 'info' });
  });
}

// Store original data for reset
const originalHospitals = hospitals.map(h => ({
  id: h.id, erCapacity: { ...h.erCapacity }, icuCapacity: { ...h.icuCapacity }, generalBeds: { ...h.generalBeds }, status: h.status,
}));
